...
<body style="background: #A8A8A8">
  <div class="right" >
        <div class="location">
            ...
            <span>通讯管理 >> 联系人修改</span>
        </div>
        <div class="providerAdd">
            <form id="userForm" name="userForm" method="post" action="index!modify.action">
				<input type="hidden" name="method" value="add">
                <!--div的class 为error是验证错误，ok是验证成功-->
                <input type="hidden" name="address.username" value="${address.username }"/>
                 <input type="hidden" name="address.ID" value="${address.ID }"/>
                <div>
                    <label for="name">好友名称：</label>
                    <input type="text" name="address.name" id="name" value="${address.name }"> 
					<!-- 放置提示信息 -->
					<font color="red"></font>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="address.email" id="email" value="${address.email}"> 
					<font color="red"></font>
                </div>
                <div>
                    <label for="qq">QQ号码</label>
                    <input type="text" name="address.qq" id="qq" value="${address.qq }"> 
					<font color="red"></font>
                </div>
                
                <div>
                    <label for="company">所在公司</label>
                    <input type="text" name="address.company" id="company" value="${address.company }"> 
					<font color="red"></font>
                </div>
                <div>
                ...
                <div>
                   ...
                   <input name="address.postcode" id="postcode"  value="${address.postcode }">
                </div>
                <div>
