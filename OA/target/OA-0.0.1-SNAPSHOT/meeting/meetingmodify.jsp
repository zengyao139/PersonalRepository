...
<body style="background: #A8A8A8">
  <div class="right" >
        <div class="location">
            ...
            <span>会议管理>> 修改会议信息</span>
        </div>
        <div class="providerAdd">
            <form id="userForm" name="userForm" method="post" action="meeting!modify.action">
            <input type="hidden" name="method" value="add">
                <!--div的class 为error是验证错误，ok是验证成功-->
                <input type="hidden" name="meeting.ID" value="${meeting.ID }"/>
                <div>
                    <label for="sender">会议填写人:</label>
                   <input type="text" id="sender" name="meeting.sender"  value="${meeting.sender }"></input>
                   <font color="red"></font>
                </div>
                <div>
                    <label for="title">开始时间:</label>
                    <input type="text" name="meeting.starttime"  onclick="WdatePicker();" id="starttime" value="${meeting.starttime }"> 
					<font color="red"></font>
                </div>
                <div>
                    <label for="content">结束时间:</label>
                    <input type="text" name="meeting.endtime" id="endtime"  onclick="WdatePicker();" value="${meeting.endtime }"> 
					<font color="red"></font>
                </div>
                <div>
                    <label for="title">会议地点:</label>
                    <input type="text" name="meeting.address" id="address" value="${meeting.address }"> 
					<font color="red"></font>
                </div>
                <div>
                    <label for="title">会议标题:</label>
                    <input type="text" name="meeting.title" id="title" value="${meeting.title }"> 
					<font color="red"></font>
                </div>
                <div>
                    <label for="content">会议内容:</label>
                    <input type="text" name="meeting.content" id="content" value="${meeting.content }"> 
					<font color="red"></font>
                </div>
